# AffirmationGenerator SDK test runner
...
class AffirmationGeneratorTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AffirmationGeneratorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AffirmationGeneratorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AffirmationGeneratorTestRunner.getenv("AFFIRMATION_GENERATOR_TEST_LIVE")
        override = AffirmationGeneratorTestRunner.getenv("AFFIRMATION_GENERATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AffirmationGeneratorTestRunner.getenv(key)
        ...
        explain = AffirmationGeneratorTestRunner.getenv("AFFIRMATION_GENERATOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if AffirmationGeneratorTestRunner._test_control is not None:
            return AffirmationGeneratorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AffirmationGeneratorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AffirmationGeneratorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AffirmationGeneratorTestRunner.load_test_control()
...
def load_env_local():
    AffirmationGeneratorTestRunner.load_env_local()


def env_override(m):
    return AffirmationGeneratorTestRunner.env_override(m)


def entity_data(v):
    return AffirmationGeneratorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AffirmationGeneratorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AffirmationGeneratorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AffirmationGeneratorTestRunner.load_test_control()


def live_client_options():
    return AffirmationGeneratorTestRunner.live_client_options()


def live_delay_ms():
    return AffirmationGeneratorTestRunner.live_delay_ms()
