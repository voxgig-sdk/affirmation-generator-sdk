# affirmation-generator-mcp

[MCP](https://modelcontextprotocol.io) server exposing the AffirmationGenerator SDK as
two agent tools — `affirmation-generator_list` and `affirmation-generator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/affirmation-generator-mcp)
make build

# 2. Provide credentials via the environment
export AFFIRMATION_GENERATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user affirmation-generator \
  -- /absolute/path/to/affirmation-generator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./affirmation-generator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// affirmation-generator_list: first page of records
{ "entity": "get_random_affirmation" }
{ "entity": "get_random_affirmation", "query": { } }

// affirmation-generator_load: one record by id
{ "entity": "get_random_affirmation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/affirmation-generator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user affirmation-generator \
     -- "$PWD"/dist/*/affirmation-generator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `affirmation-generator_list` and `affirmation-generator_load` tools now appear
   in new sessions. Ask the agent to *"list get_random_affirmation using affirmation-generator"*
   and it calls `affirmation-generator_list` with `{"entity":"get_random_affirmation"}`.
...
```sh
export AFFIRMATION_GENERATOR_APIKEY=sk_live_xxx            # API key
export AFFIRMATION_GENERATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `affirmation-generator_list` tool
...
### Call the `affirmation-generator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `affirmation-generator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `affirmation-generator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AFFIRMATION_GENERATOR_APIKEY` | API key sent with every request. |
| `AFFIRMATION_GENERATOR_BASE` | Optional override of the API base URL. |
...
```sh
./affirmation-generator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"affirmation-generator_load","arguments":{"entity":"get_random_affirmation","query":{"id":1}}}}'
```
