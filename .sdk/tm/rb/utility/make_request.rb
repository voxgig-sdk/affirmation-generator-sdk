...
module AffirmationGeneratorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AffirmationGeneratorResponse.new({})
    ...
    if fetch_err
      ...
      response = AffirmationGeneratorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AffirmationGeneratorResponse.new(fetched)
    else
