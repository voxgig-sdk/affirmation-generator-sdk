...
// AffirmationGenerator SDK feature test
//
...
class FtCtrl extends AffirmationGeneratorControl
{
...
class FtClient
{
    ...
    private AffirmationGeneratorUtility $utility;

    public function __construct(AffirmationGeneratorUtility $utility, array $options)
    {
    ...
    public function get_utility(): AffirmationGeneratorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AffirmationGeneratorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AffirmationGeneratorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AffirmationGeneratorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AffirmationGeneratorConfig::make_feature($name);
    ...
    private static function buildUrl(AffirmationGeneratorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AffirmationGeneratorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AffirmationGeneratorResponse($fetched);
        }
    ...
    private function fail(AffirmationGeneratorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AffirmationGeneratorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
