...
// AffirmationGenerator SDK pipeline test
//
...
{
    private static function utility(): AffirmationGeneratorUtility
    {
        return new AffirmationGeneratorUtility();
    }
    ...
    private static function ctx(array $over = []): AffirmationGeneratorContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AffirmationGeneratorContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AffirmationGeneratorOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AffirmationGeneratorError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AffirmationGeneratorOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AffirmationGeneratorMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AffirmationGeneratorOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AffirmationGeneratorMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AffirmationGeneratorOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AffirmationGeneratorMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AffirmationGeneratorSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AffirmationGeneratorMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AffirmationGeneratorResponse([]),
            'result' => new AffirmationGeneratorResult([]),
        ]);
        [, $err] = AffirmationGeneratorMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AffirmationGeneratorSpec([]),
            'response' => null,
            'result' => new AffirmationGeneratorResult([]),
        ]);
        [, $err] = AffirmationGeneratorMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AffirmationGeneratorSpec(['step' => 's']),
            'response' => new AffirmationGeneratorResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AffirmationGeneratorResult([]),
        ]);
        [, $err] = AffirmationGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AffirmationGeneratorSpec(['step' => 's']),
            'response' => new AffirmationGeneratorResponse(self::resp(200, ['v' => 1])),
            'result' => new AffirmationGeneratorResult([]),
        ]);
        [, $err] = AffirmationGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AffirmationGeneratorSpec(['step' => 's']),
            'response' => new AffirmationGeneratorResponse(self::resp(200, ['v' => 2])),
            'result' => new AffirmationGeneratorResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AffirmationGeneratorMakeResponse::call($ctx);
    ...
    {
        $preset = new AffirmationGeneratorResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AffirmationGeneratorSpec([]),
            'response' => new AffirmationGeneratorResponse([]),
            'result' => new AffirmationGeneratorResult([]),
        ...
        [$got, $err] = AffirmationGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AffirmationGeneratorResult([])]);
        [, $err] = AffirmationGeneratorMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AffirmationGeneratorSpec([]), 'result' => null]);
        [, $err] = AffirmationGeneratorMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AffirmationGeneratorSpec(['step' => 's']),
            'result' => new AffirmationGeneratorResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AffirmationGeneratorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AffirmationGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AffirmationGeneratorSpec(['step' => 's']),
            'result' => new AffirmationGeneratorResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AffirmationGeneratorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AffirmationGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AffirmationGeneratorResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AffirmationGeneratorSpec([]),
            'result' => new AffirmationGeneratorResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AffirmationGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AffirmationGeneratorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AffirmationGeneratorError('boom', 'boom');
        $utility->fetcher = function (AffirmationGeneratorContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AffirmationGeneratorSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AffirmationGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AffirmationGeneratorContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AffirmationGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AffirmationGeneratorResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        AffirmationGeneratorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AffirmationGeneratorContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AffirmationGeneratorSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AffirmationGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AffirmationGeneratorResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AffirmationGeneratorSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AffirmationGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AffirmationGeneratorMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AffirmationGeneratorMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AffirmationGeneratorResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AffirmationGeneratorDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AffirmationGeneratorResult(['ok' => false])]);
        $this->expectException(AffirmationGeneratorError::class);
        AffirmationGeneratorDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AffirmationGeneratorResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AffirmationGeneratorMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AffirmationGeneratorMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AffirmationGeneratorBaseFeature();
        $b = new AffirmationGeneratorBaseFeature();
        AffirmationGeneratorFeatureAdd::call($ctx, $a);
        AffirmationGeneratorFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AffirmationGeneratorBaseFeature
    {
        $f = new AffirmationGeneratorBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AffirmationGeneratorFeatureAdd::call($ctx, self::named_feature('a'));
        AffirmationGeneratorFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        AffirmationGeneratorFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        AffirmationGeneratorFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        AffirmationGeneratorFeatureAdd::call($ctx, $replace);
        ...
        AffirmationGeneratorFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AffirmationGeneratorContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AffirmationGeneratorSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AffirmationGeneratorPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        AffirmationGeneratorPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AffirmationGeneratorResponse(['headers' => null]),
            'result' => new AffirmationGeneratorResult([]),
        ]);
        AffirmationGeneratorResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        AffirmationGeneratorResultBody::call($ctx);
