...
// AffirmationGenerator SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AffirmationGeneratorOmni::makeStructRunner(
    ...
    {
        return AffirmationGeneratorOmni::objmodel(AffirmationGeneratorOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return AffirmationGeneratorOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return AffirmationGeneratorOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AffirmationGeneratorOmni::structfix($now, Struct::undef()))
                        !== json_encode(AffirmationGeneratorOmni::structfix($val, Struct::undef()))) {
