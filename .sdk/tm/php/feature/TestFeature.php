...
// AffirmationGenerator SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AffirmationGeneratorTestFeature extends AffirmationGeneratorBaseFeature
{
    ...
    public function init(AffirmationGeneratorContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AffirmationGeneratorContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AffirmationGeneratorParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AffirmationGeneratorContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AffirmationGeneratorContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AffirmationGeneratorParam::call($ctx, $k);
