...
// AffirmationGenerator SDK retry feature
...
class AffirmationGeneratorRetryFeature extends AffirmationGeneratorBaseFeature
{
    ...
    public function init(AffirmationGeneratorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AffirmationGeneratorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AffirmationGeneratorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AffirmationGeneratorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
