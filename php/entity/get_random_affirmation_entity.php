...
// AffirmationGenerator SDK GetRandomAffirmation entity
...
class GetRandomAffirmationEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "get_random_affirmation";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new GetRandomAffirmationEntity($this->_client, $opts);
    }

    /**
     * @param GetRandomAffirmation|array $args GetRandomAffirmation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AffirmationGeneratorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetRandomAffirmation|array The current GetRandomAffirmation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetRandomAffirmation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AffirmationGeneratorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetRandomAffirmation fields).
     */
    ...
    /**
     * Load a single GetRandomAffirmation.
     *
     * @param GetRandomAffirmationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetRandomAffirmationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetRandomAffirmation|array The loaded GetRandomAffirmation as an assoc-array at the
     *   SDK boundary; throws AffirmationGeneratorError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = AffirmationGeneratorHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
