...
class GetRandomAffirmationEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = AffirmationGeneratorSDK::test(null, null);
        $ent = $testsdk->GetRandomAffirmation(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = get_random_affirmation_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "get_random_affirmation." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set AFFIRMATION_GENERATOR_TEST_GET_RANDOM_AFFIRMATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $get_random_affirmation_ref01_data = null;
        if (count($get_random_affirmation_ref01_data_raw) > 0) {
...
function get_random_affirmation_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/get_random_affirmation/GetRandomAffirmationTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["get_random_affirmation01", "get_random_affirmation02", "get_random_affirmation03"] as $k) {
    ...
    $entid_env_raw = getenv("AFFIRMATION_GENERATOR_TEST_GET_RANDOM_AFFIRMATION_ENTID");
    ...
    if ($env["AFFIRMATION_GENERATOR_TEST_LIVE"] === "TRUE") {
        ...
        $client = new AffirmationGeneratorSDK(Helpers::to_map($merged_opts));
    }
