...
//    only - a typed AffirmationGeneratorContext there would make every ctx
...
final class AffirmationGeneratorOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AffirmationGeneratorContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new AffirmationGeneratorSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new AffirmationGeneratorResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new AffirmationGeneratorError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new AffirmationGeneratorResponse($clean['response']);
    ...
    public static function err_from_map($m): ?AffirmationGeneratorError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AffirmationGeneratorError(is_string($code) ? $code : '', $msg);
    }
