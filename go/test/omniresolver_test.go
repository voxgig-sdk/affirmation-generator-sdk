...
import (
	...
	omni "github.com/voxgig-sdk/affirmation-generator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AffirmationGeneratorError); is {
				out := map[string]any{
					"name":    "AffirmationGeneratorError",
...
func omniCtx(arg any, client *sdk.AffirmationGeneratorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AffirmationGeneratorSDK); is {
			client = live
