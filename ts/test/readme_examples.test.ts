...
//     (\`new AffirmationGeneratorSDK(...)\` / \`AffirmationGeneratorSDK.test(...)\` become
//     \`AffirmationGeneratorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AffirmationGeneratorSDK'
...
const TEST_SEED = {"entity":{"get_random_affirmation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AffirmationGeneratorSDK(...)\` and any
// \`AffirmationGeneratorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AffirmationGeneratorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AffirmationGeneratorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
