# affirmation-generator-cli

boru-driven command-line client **and** interactive REPL for the AffirmationGenerator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/affirmation-generator-cli)
make build

# 2. See usage (words, entities, env vars)
./affirmation-generator-cli --help

# 3. Provide credentials once, via the environment
export AFFIRMATION_GENERATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./affirmation-generator-cli load 1 get_random_affirmation            # {id:1} shorthand
./affirmation-generator-cli load '{id:1}' get_random_affirmation       # explicit match map

# 5. Override the API base URL for a single call
AFFIRMATION_GENERATOR_BASE=https://api.example.com ./affirmation-generator-cli load 1 get_random_affirmation

# 6. No arguments -> interactive REPL
./affirmation-generator-cli
affirmation-generator> load 1 get_random_affirmation
affirmation-generator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/affirmation-generator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/affirmation-generator-cli load 1 get_random_affirmation
   ```
...
```sh
./affirmation-generator-cli load 1 get_random_affirmation          # scalar shorthand for {id:1}
./affirmation-generator-cli load '{id:1}' get_random_affirmation     # explicit match map
```
...
./affirmation-generator-cli load 1 get_random_affirmation
```
...
Run with no arguments to open a REPL (prompt `affirmation-generator>`). Each line is
evaluated as its own boru expression:

```text
$ ./affirmation-generator-cli
affirmation-generator> load 1 get_random_affirmation
affirmation-generator> /help
affirmation-generator> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_random_affirmation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/affirmation-generator-cli`. |
...
(`./affirmation-generator-cli <expr>`) and interactively (the REPL), and expressions compose the
