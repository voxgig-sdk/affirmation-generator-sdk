// Command AffirmationGenerator-cli is a CLI + REPL for the AffirmationGenerator SDK,
...
const prompt = "affirmation-generator"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_random_affirmation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AFFIRMATION_GENERATOR_APIKEY (api key), AFFIRMATION_GENERATOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AFFIRMATION_GENERATOR_APIKEY carries the API key and
	// AFFIRMATION_GENERATOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AFFIRMATION_GENERATOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AFFIRMATION_GENERATOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAffirmationGeneratorSDK(opts)
