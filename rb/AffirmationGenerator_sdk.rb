...
require_relative 'AffirmationGenerator_types'
...
class AffirmationGeneratorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AffirmationGeneratorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AffirmationGeneratorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AffirmationGeneratorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AffirmationGeneratorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AffirmationGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AffirmationGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AffirmationGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => AffirmationGeneratorError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AffirmationGeneratorError => err
    ...
    if fetched.is_a?(Hash)
      status = AffirmationGeneratorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AffirmationGeneratorError.new(
        "graphql_error", "AffirmationGeneratorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AffirmationGeneratorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
