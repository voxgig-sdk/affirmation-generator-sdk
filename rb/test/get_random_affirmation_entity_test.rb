...
class GetRandomAffirmationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set AFFIRMATION_GENERATOR_TEST_GET_RANDOM_AFFIRMATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_random_affirmation_ref01_data_dt0_loaded = get_random_affirmation_ref01_ent.load(get_random_affirmation_ref01_match_dt0, nil)
...
def get_random_affirmation_basic_setup(extra)
  ...
  entid_env_raw = ENV["AFFIRMATION_GENERATOR_TEST_GET_RANDOM_AFFIRMATION_ENTID"]
