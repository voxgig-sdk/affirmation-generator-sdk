...
class GetRandomAffirmationEntity
  ...
  def make
    opts = @_entopts.dup
    GetRandomAffirmationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AffirmationGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetRandomAffirmation, Hash] the current GetRandomAffirmation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AffirmationGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetRandomAffirmation fields)
  def match_get
  ...
  #
  # @param reqmatch [GetRandomAffirmationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetRandomAffirmation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetRandomAffirmation, Hash] the loaded GetRandomAffirmation; raises AffirmationGeneratorError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AffirmationGeneratorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
